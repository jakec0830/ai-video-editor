...
**審片（一次改完，不要一句一句回）**：AI 給你 preview 後，與其一句一句回「這裡改那裡改」，不如用審片工具一次收齊。雙擊打開工具包根目錄的 `審片.html`，把 preview 影片拉進去，在想改的地方暫停打字（下面有詞塊可以快速輸入：太緊、太鬆、推進、蓋到臉…），全部標完按「複製全部」，整段貼回給 AI，它一次改完再重出一版。這樣快、也省 token。要標「位置」（字幕擺哪、東西放哪）時，開「✎ 畫筆」直接在畫面上圈，送出後按「下載畫記圖」，把下載的圖跟文字一起給 AI。
