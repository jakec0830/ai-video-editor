...
- `lang="zh-Hant"`。**字幕字型給兩個預設,用中文名讓使用者選**(第一次問,選好記進偏好檔,之後不用再問):
  - **宋體** — 有襯線、優雅、文青感 → font-family `"Source Han Serif TC VF", "Source Han Serif TC"`(繁中一定要 **TC**。Mac 裝的是 VF 變數字型(family 有 VF),Windows 裝的是靜態子集(family 沒 VF),**兩個名字都列上去才能跨平台不出包**;`"Source Han Serif VF"` 這個沒 TC 的 family 不存在,會 fallback 成黑體)
  - **黑體** — 無襯線、粗、有力、短影音最常見 → font-family `"PingFang TC", "Microsoft JhengHei"`(PingFang 是 Mac,正黑體是 Windows),`font-weight:700`
  兩個都用 `@font-face { font-family:"<家族名>"; src: local("<家族名>"); }`(列到的每個 local 家族名各給一條)。**關鍵字 highlight 只換顏色(黃),字型跟整句一模一樣,絕不換字型**
