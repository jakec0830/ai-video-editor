...
為了把工具做得更好用，這個工具包會把**匿名的使用/錯誤紀錄**（你剪片時卡在哪、哪個步驟出錯、安裝順不順）回傳給課程團隊。送出前會自動洗掉你電腦的帳號名稱，**不會傳你的影片、密碼或個資**。這些紀錄的原始檔都存在你自己的 `錯誤回報/` 資料夾，你隨時看得到。不想回傳的話，跟 AI 說一聲「不要回傳紀錄」就會停。
...
- **Mac**：蘋果選單 → 關於這台 Mac → 確認 macOS 是 **13 以上**、儲存空間剩 **30GB** 以上。macOS 12 以下沒辦法上這門課（Claude Code 跑不起來），**先不要訂閱**。
- **Windows**：Windows 10/11 都可以，不用檢查。

**二、訂閱跟裝 App（你自己做，全是點按鈕）**

- 到 claude.ai 開通 **Claude 訂閱（Pro 或 Max）**。
- 下載安裝 **Claude 桌面版 App**（到 claude.ai 下載 Mac / Windows 版），一般安裝、登入。它本身就內建 Claude Code，你不用另外裝 Node 或 CLI。

**三、貼一行指令（整個流程唯一一次碰終端機）**

這行會幫你裝 git、把工具包下載到你的家目錄。照做就好，不用懂它。

- **Mac**：按 Cmd+空白鍵 → 打「終端機」→ Enter。把下面整行貼進去按 Enter：

  ```
  curl -fsSL https://raw.githubusercontent.com/jakec0830/ai-video-editor/main/bootstrap.sh | bash
  ```

  中途會跳出視窗問要不要裝「命令列開發者工具」→ 點「安裝」，等它跑完（10~30 分鐘，畫面會一直告訴你進度）。

- **Windows**：按 Windows 鍵 → 打「powershell」→ Enter。把下面整行貼進去按 Enter：

  ```
  irm https://raw.githubusercontent.com/jakec0830/ai-video-editor/main/bootstrap.ps1 | iex
  ```

  中途跳出「要允許此 App 變更你的裝置嗎？」→ 點「是」。

  結尾看到「✅ 全部完成」就成功了，視窗可以關掉。卡住的話，把整個視窗截圖傳到課程群組。

**四、開始設定**

1. 打開 App，點上面的 **Code** 分頁（App 本來就開著的話，先完全關掉重開一次）。
2. 環境選 **Local**（在你自己電腦上跑，才能處理你的影片。不要選 Cloud）。
3. 用 **Select folder（選資料夾）** 選家目錄裡的 **`ai-video-editor` 資料夾本身**（不是桌面、不是文件）。選對了的話，新對話的 AI 會知道自己是剪片助理；選錯了它會像個普通聊天。之後每次用也都是開這個資料夾。
4. 在對話框打：

   > 幫我一步一步安裝設定

5. 照它說的做。**所有指令都是 AI 跑，你不用再碰終端機。** 它會檢查你缺什麼（Node、ffmpeg、字型等）一次講清楚再幫你裝，需要你動手的只有：在 App 裡按「允許」、偶爾在跳出的視窗點個按鈕。每到這種步驟它會停下來說「這步換你做」。
